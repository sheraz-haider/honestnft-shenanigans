...
def gen_rarity_score(trait_db, trait_types, method, trait_count, sum_traits, sum_trait_multiplier):
    ...
    if method == 'raritytools':
        ...
        if trait_count:
            ...
            trait_db['NUM_TRAITS'] = rarity_db.apply(
                lambda row: len(trait_types) - sum(row[0:(len(rarity_db.columns))] == 'None'),
                axis=1)
        ...
        if len(sum_traits) > 0:
            ...
            for trait in sum_traits:
                ...
                rarity_db[scaled_trait] = (rarity_db[trait] - trait_min) / (trait_max - trait_min)
            ...
            rarity_db['SUM_TRAIT'] = rarity_db[scaled_traits].sum(axis=1) * multiplier * sum_trait_multiplier
...
def build_rarity_db(collection, attribute_file, method, trait_count, sum_traits, sum_trait_multiplier):
...
if __name__ == '__main__':
    ...
    ARG_PARSER.add_argument('-method', type=str, default='raritytools', help='Method to use to compute rarity. (default: raritytools)')
    ARG_PARSER.add_argument('-trait_count', type=bool, default=True, help='Toggle using trait count in computation. (default: True)')
    ARG_PARSER.add_argument('-sum_traits', type=str, nargs='+', help='Traits to sum instead of computing rarity. Can be one or many. (default: None)')
    ARG_PARSER.add_argument('-sum_trait_mult', type=float, default=35, help='Trait score multiplier to use for summed traits. (default: 35)')
